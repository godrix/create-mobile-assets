...
async function generateAssets(pathBackgroud, pathDevice, screenSize = [1090, 1940], folder, fileName, behind = false, backgroundColor, positionHeight) {
...
try {
  ...
  Promise.all(filesMobile).then((data) => {
    ...
    for (let index = 0; index < screenshots.length; index++) {
      ...
      backgroud.print(font, backgroudSize[0]/2 - textWidth/2, 0, 'text')
