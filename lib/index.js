...
const { choiceColor, choicePosition } = require('../utils/prompt')
const { FOLDER_SCREEENSHOT, PATH_RESULT } = require('../constants');

async function generateAssets(pathBackgroud, pathDevice, screenSize = [1090, 1940], folder, fileName, behind = false) {

try {

  const backgroud = await jimp.read(`${pathBackgroud}/bg.png`);
  const backgroundColor = await choiceColor();
  ...
  const positionHeight = await choicePosition();
